            ...
            using (var channel = Channel.Create(ChannelDriver.Create(ChannelBridge.Create(), MyoErrorHandlerDriver.Create(MyoErrorHandlerBridge.Create()))))
            {
                using (var hub = Hub.Create(channel))
                {
                    hub.MyoConnected += (sender, e) =>
                    {
                        ...
                        {
                            var myo = (Myo)o;
                            Console.WriteLine(PixelFromVector(DirectionalVector(FromQuaternion(myo.Orientation))));
                        ...
                        e.Myo.PoseChanged += (o, args) =>
                        {
                            ...
                            var window = api.WindowFromPoint(PixelFromVector(DirectionalVector(FromQuaternion(myo.Orientation))));
